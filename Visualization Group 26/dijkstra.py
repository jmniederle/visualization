...
def callDijkstra(node_1, node_2, df):
    ...
    for key in dct:
        try:
            test = Graph(dct[key])
            outputdct[key] = test.dijkstra(node_1, node_2)
        except:
            errorlist.append(key)
    return outputdct
 
if __name__ == '__main__':
    df = pd.read_csv('profile_semantic_trafo_final.txt', sep='\s')          
    output = callDijkstra(515,843,df)
